...
*Page 1 — Platform Health: KPI cards, value by risk tier, monthly value trend. Date slicer (Jan 2017–Aug 2018). HIGH Risk Sellers and Value At Risk are isolated from the slicer by design.*

<img src="BI/Seller Distribution & Pareto Analysis.png" width="920"/>

*Page 2 — Distribution & Pareto: review score and late delivery distributions, Pareto curve with 50% and 80% reference lines, Spearman r = −0.46, and on-time delivery rate.*
...
Orders on Olist can have multiple sellers. `int_order_facts` assigns each order to the seller with the highest value contribution using `ROW_NUMBER()`.
...
Thresholds in `mart_seller_risk` are grounded in the actual distribution of seller performance metrics, filtered to sellers with `delivered_orders >= 10`.
...
- `MEDIUM < 3.8` — below P25, floor for underperforming sellers

**Late delivery thresholds:**
- `MEDIUM > 0.10` — above P75, elevated vs peers
- `HIGH > 0.30` — over 3× above P75, severe outlier

Thresholds are designed to flag only clear outliers. The dual-failure condition for HIGH (`late_delivery_rate > 0.30 AND avg_review_score < 3.5`) requires both dimensions to deteriorate simultaneously, reducing false positives from our data.
